                ...
                if (action?.workspace?.id) {
                    try {
                        const uploadResponse = await agent.uploadFile({
                            ...
                            path: `reports/${args.address.toLowerCase()}_${timestamp}.md`,
                            file: Buffer.from(analysisContent, 'utf-8'), // Convert to Buffer
                        ...
                        if (action.task?.id) {
                            await agent.addLogToTask({
                                ...
                                severity: 'info',
                                type: 'text',
                                body: `📄 Analysis report saved: ${uploadResponse.fullUrl}`
                            });
                        }
                    } catch (uploadError) {
                        console.error('Error uploading analysis file:', uploadError);
                        // Add error log but continue execution
                        if (action.task?.id) {
                            await agent.addLogToTask({
                                workspaceId: action.workspace.id,
                                taskId: action.task.id,
...
Generated by EthWalletAgent`;
                    ...
                    try {
                        const uploadResponse = await this.uploadFile({
                            workspaceId: action.workspace.id,
                            path: `reports/${addressMatch[0].toLowerCase()}_${timestamp}.md`,
                            file: Buffer.from(analysisContent, 'utf-8'),
                            taskIds: [task.id]
                        });
