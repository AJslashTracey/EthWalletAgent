    ...
    async run({ args, action }, messages) {
        try {
            ...
            if (result.chatGPTResponse) {
                // Save analysis if we have workspace context
                if (action?.workspace?.id) {
                    try {
                        ...
                        const filePath = `wallet_analysis_${args.address.toLowerCase()}.md`;

                        console.log('Uploading analysis to workspace:', {
                            workspaceId: parseInt(action.workspace.id),
                            path: filePath,
                            hasTask: !!action.task?.id
                        });
                        ...
                        if (action.task?.id) {
                            await agent.addLogToTask({
                                ...
                                severity: 'info',
                                type: 'text',
                                body: `✅ Analysis saved as: ${filePath}`
                            });
                        }
                    } catch (uploadError) {
            ...
            if (responseText) {
                ...
                if (addressMatch) {
                    ...
                    try {
                        ...
                        const filePath = `wallet_analysis_${addressMatch[0].toLowerCase()}.md`;
