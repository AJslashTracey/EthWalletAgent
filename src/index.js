...
Generated by EthWalletAgent`;
                        ...
                        formData.append('path', `wallet_analysis/${args.address.toLowerCase()}.md`);
                        formData.append('file', Buffer.from(markdownContent));
                        
                        if (action.task?.id) {
                            formData.append('taskIds', JSON.stringify([action.task.id]));
                        }
                        
                        formData.append('skipSummarizer', 'false');

                        await agent.uploadFile({
                            workspaceId: action.workspace.id,
                            path: `wallet_analysis/${args.address.toLowerCase()}.md`,
                            file: Buffer.from(markdownContent),
                            taskIds: action.task?.id ? [action.task.id] : undefined,
                            skipSummarizer: false
                        });

                        // Log if part of a task
                        if (action.task?.id) {
                            await agent.addLogToTask({
                                ...
                                severity: 'info',
                                type: 'text',
                                body: `✅ Analysis saved as: analysis_${args.address.toLowerCase()}_${timestamp}.md`
                            });
                        }
                    } catch (error) {
...
Generated by EthWalletAgent for Task #${task.id}`;
                    ...
                    try {
                        await this.uploadFile({
                            workspaceId: action.workspace.id,
                            path: `wallet_analysis/${addressMatch[0].toLowerCase()}.md`,
                            file: Buffer.from(markdownContent),
                            taskIds: [task.id],
                            skipSummarizer: false
                        });
                        ...
                        await this.addLogToTask({
                            ...
                            severity: 'info',
                            type: 'text',
                            body: `✅ Analysis saved as: analysis_${addressMatch[0].toLowerCase()}_${timestamp}.md`
                        });
                    } catch (error) {
                        console.error('File upload error:', {
                            error: error.message,
