...
Always maintain context between messages and remember previously provided addresses.`,
    autoPersist: true // Enable automatic file persistence
});

// Helper function to generate unique filenames
function generateAnalysisFileName(address) {
    const date = new Date().toISOString().slice(0,10);
    return `${date}_${address}_analysis.md`;
}
...
// ...existing markdown template...`;
                ...
                console.error("[doTask] File save error:", fileError);
                throw new Error(`File operation failed: ${fileError.message}`);
            }
        } else {
